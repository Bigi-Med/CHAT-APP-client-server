...
void add_end(struct connection_information *client,int client_fd,char *ip, int port,char *pseudo_name,char *timer)
{
...
void multi_join(int sockfd,struct message msg,char *buff,struct connection_information *client,struct salon *salle)
{
	...
	struct salon *q;
	q = salle;
	char this_salon[NICK_LEN];
	memset(this_salon,0,NICK_LEN);
	strncpy(this_salon,msg.infos,strlen(msg.infos));
	printf("this sallon is %s \n",this_salon);
	while(p!=NULL)
	{
		if(p->socket_fd == sockfd)
		{
			...
			{
				memset(&msg,0,sizeof(msg));
				printf("in cond first time \n");
				strncpy(msg.infos,p->salon_name,strlen(p->salon_name));
				multi_quit(sockfd,msg,buff,client,salle);
				continue;
			}
			else{
			p->salon_name = this_salon;
			printf("salon name is %s \n",p->salon_name);
			
			while(q!=NULL)
			{
				if(!(strncmp(q->nom_salon,this_salon,MIN(strlen(q->nom_salon),strlen(this_salon)))))
				{
...
void file_send(struct message msg,struct connection_information *client,int sockfd)
{
	char buff[MSG_LEN];
	memset(buff,0,MSG_LEN);
	if(recv(sockfd,buff,msg.pld_len,0)<0)
	{
		perror("Error while recv file name ");
		exit(EXIT_FAILURE);
	}
	...
	strncpy(destination,msg.infos,strlen(msg.infos));
	strncpy(source,msg.nick_sender,strlen(msg.nick_sender));
	memset(&msg,0,sizeof(msg));
	msg.type = FILE_REQUEST;
	strncpy(msg.infos,buff,strlen(buff));
	strncpy(msg.nick_sender,source,strlen(source));
	//msg.pld_len  = 0;
	//RECV file name
	...
	while(p!=NULL)
	{
		...
		if(strcmp(p->pseudo,destination)!=0)
		{
			...
			//send file request struct 
			
			if(send(p->socket_fd,&msg,sizeof(msg),0)<0)
			{
				perror("Error while sending file requ");
				exit(EXIT_FAILURE);
			}
...
void file_accept(struct message msg,struct connection_information *client,int sockfd)
{
	char buff[MSG_LEN];
	memset(buff,0,MSG_LEN);
	if(recv(sockfd,buff,msg.pld_len,0)<0)
	{
		perror("Error while receiving acc msg");
		exit(EXIT_FAILURE);
	}
	//sending struct to source client
	struct connection_information *p;
	p = client;
	while(p!=NULL)
	{
		if(strcmp(p->pseudo,msg.infos)!=0)
		{
			p = p->next;
			continue;
		}
			...
			if(send(p->socket_fd,&msg,sizeof(msg),0)<0)
			{
				perror("Error while sending acc struct");
				exit(EXIT_FAILURE);
			}
			//sending payload to client
			if(send(p->socket_fd,buff,msg.pld_len,0)<0)
			{
				perror("Error while sending acc msg ");
				exit(EXIT_FAILURE);
			}
			p=p->next;
			continue;
...
void file_reject(struct message msg,struct connection_information *client,int sockfd)
{
	struct connection_information *p;
	p =client;
	...
	while(p!=NULL)
	{
		if(strcmp(p->pseudo,msg.infos)!=0)
		{
			...
			if(send(p->socket_fd,&msg,sizeof(msg),0)<0)
			{
				perror("Error while sending file reject struct ");
				exit(EXIT_FAILURE);
			}
			break;
		}
		...
		for (int i = 0; i < nfds; i++) {
			...
			if (pollfds[i].fd == listen_fd && pollfds[i].revents & POLLIN) {
				...
				if ((client_fd > 4)|| (cach >= 2 && client_fd == 4))
				{
					...
					time_t t;
					time(&t);
					add_end(client_head,client_fd,inet_ntoa(client_address),ntohs(sockptr->sin_port),nick,ctime(&t));
					//free(nick);
				}
				...
				if(msgstruct.type == FILE_REQUEST)
				{
					...
					file_send(msgstruct,client_head,pollfds[i].fd);
				}
				if(msgstruct.type == FILE_ACCEPT)
				{
				
					file_accept(msgstruct,client_head,pollfds[i].fd);
				}
				if(msgstruct.type == FILE_REJECT)
				{
					
					file_reject(msgstruct,client_head,pollfds[i].fd);
				}
				//receiving message
